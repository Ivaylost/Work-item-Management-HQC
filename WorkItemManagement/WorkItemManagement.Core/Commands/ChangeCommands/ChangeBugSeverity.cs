...
{
    public class ChangeBugSeverity :  ICommand
    {
        private readonly IDatabase database;
        private readonly IWIMFactory factory;

        public ChangeBugSeverity(IWIMFactory factory, IDatabase database)
        {
        ...
        public  string Execute(IList<string> parameters)
        {
            ...
            string newSeverity = parameters[3];
            ...
            var bug = board.ListOfWorkItems
                .OfType<IBug>()
                .FirstOrDefault(y => y.Title == bugTitle);

            bug.ChangeSeverity(newSeverity);

            string result = string.Format(GlobalConstants.BugSeverityWasChanged, bugTitle, newSeverity);
