...
namespace WorkItemManagement.Core.Commands.ChangeCommands
{
    public class ChangeTaskPriority :  ICommand
    {
        ...
        public ChangeTaskPriority(IWIMFactory factory, IDatabase database)
        {
        ...
        public  string Execute(IList<string> parameters)
        {
            ...
            string taskTitle = parameters[2];
            string newPriority = parameters[3];
            ...
            var task = board.ListOfWorkItems
                .OfType<ITask>()
                .FirstOrDefault(y => y.Title == taskTitle);

                task.ChangePriority(newPriority);
           

            string result = string.Format(GlobalConstants.PriorityWasChanged, taskTitle, newPriority);
